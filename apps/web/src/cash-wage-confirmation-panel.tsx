...
import { ApiClientError, RoleSelectionRequiredError, StaleResponseError,
  ...
  type SalaryBenefitDocumentSubmission } from "@teaching-research-alliance/client";
...
export function CashWageConfirmationPanel({ client, session, sessionKey, busy = false, onInvalidated, onSaved, onUnconfirmedChange }: Props): ReactNode {
  ...
  const resetDraft = (): void => {
    documentRef.current = null; createCommand.current = null; confirmCommand.current = null;
  ...
  const upload = async (action: () => Promise<unknown>, id: string): Promise<void> => {
    if (running.current || busy || phase !== "idle" || documentRef.current !== id) return;
    const token = generation.current; running.current = true; setUploading(true);
    try { await action(); if (token === generation.current && documentRef.current === id) await readAttachments(id, token); }
  ...
  if (!authorized) return <section className="panel" aria-label="现金工资确认"><h2>现金工资确认</h2><p>当前身份没有确认现金工资的权限。</p></section>;
  const token = generation.current;
  return <section className="panel" aria-label="现金工资确认"><h2>现金工资确认</h2>
    <label>工资月<input aria-label="确认工资月" type="month" value={month.slice(0, 7)} disabled={locked} onChange={event => setMonth(`${event.target.value}-01`)} /></label>
    {loading && <p role="status">正在读取工资待办…</p>}
    {!loading && roster === null && <button type="button" onClick={() => void readRoster(generation.current)}>重新读取工资待办</button>}
    {roster !== null && <><label>老师<select aria-label="确认工资老师" value={selected} disabled={locked} onChange={event => {
      generation.current += 1; running.current = false; resetDraft(); setSelected(event.target.value); setMessage("");
    }}><option value="">请选择有待办的老师</option>{roster.items.filter(row => row.todo !== null).map(row => <option key={row.teacherPersonId} value={row.teacherPersonId}>{row.teacherDisplayName} · 计划 {yuan(row.plan.plannedCashCents)} 元 · 已确认 {yuan(row.confirmedCashCents)} 元 · 剩余 {yuan(row.remainingCashCents)} 元</option>)}</select></label>
    {item && <><p>计划现金 {yuan(item.plan.plannedCashCents)} 元；已确认 {yuan(item.confirmedCashCents)} 元；剩余 {yuan(item.remainingCashCents)} 元。</p>
      <label>实际现金（元）<input value={cash} disabled={locked} onChange={event => setCash(event.target.value)} /></label>
      <label>扣豆（欢乐豆）<input value={deduction} disabled={locked} onChange={event => setDeduction(event.target.value)} /></label>
      <label><input type="checkbox" checked={paid} disabled={locked} onChange={event => setPaid(event.target.checked)} />我确认现金已在线下发放</label>
      <label>理由<textarea value={reason} disabled={locked} onChange={event => setReason(event.target.value)} /></label>
      {document === null ? <button type="button" disabled={busy || !["idle", "create-unknown"].includes(phase)} onClick={() => void createDocument()}>{phase === "create-unknown" ? "安全重试创建凭证" : "创建工资确认凭证"}</button> : <>
        {purposes.map(purpose => <AttachmentPicker key={`${document.id}:${purpose}`} client={client} documentId={document.id} purpose={purpose} label={purpose === "SUPPORTING_DOCUMENT" ? "工资发放凭证" : "工资确认截图"}
          disabled={busy || uploading || phase !== "idle" || purposes.some(other => other !== purpose && pendingUploads[other])}
          run={action => upload(action, document.id)}
          onReady={id => { if (generation.current === token && documentRef.current === document.id) setReady(current => ({ ...current, [purpose]: id })); }}
          onPendingChange={pending => { if (generation.current === token && documentRef.current === document.id) setPendingUploads(current => ({ ...current, [purpose]: pending })); }} />)}
        {attachments === null && <button type="button" disabled={locked} onClick={() => { const current = generation.current; void readAttachments(document.id, current).catch(error => { if (current === generation.current) fail(error); }); }}>重新读取凭证附件</button>}
        {["conflict", "reconciling"].includes(phase) ? <button type="button" disabled={phase === "reconciling" || busy} onClick={() => void reconcile()}>重新读取并核对</button> : <button type="button" disabled={busy || loading || uploading || hasPendingUpload || !["idle", "confirm-unknown"].includes(phase)} onClick={() => void confirm()}>{phase === "confirm-unknown" ? "安全重试原确认" : "确认现金已发放并记录扣豆"}</button>}
      </>}
    </>}
    </>}
