...
export function BonusProjectPanel({ client, session, sessionKey, onInvalidated, onSaved, onUnconfirmedChange }: Props): ReactNode {
  ...
  const save = async (project: Row): Promise<void> => {
    ...
    if (!canWrite || conflict[number] || activeWrite.current !== null || commandOwner.current !== null && commandOwner.current !== number) return;
