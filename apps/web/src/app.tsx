...
function App(){
  ...
  return <div className="shell"><aside><div className="brand">研<span>教研联盟</span></div><p>让每一份教学付出<br/>都有清楚的记录。</p><div className="nav">我的教学</div><small>个人账户 · 费用记录</small></aside><main>
    <header><div><span className="eyebrow">TEACHING ALLIANCE</span><h1>{session?"我的教学":"欢迎回来"}</h1></div>{session&&<button className="quiet" disabled={busy} onClick={()=>{client.logout();clear();setSession(null);setMessage("");}}>退出此页面</button>}</header>
