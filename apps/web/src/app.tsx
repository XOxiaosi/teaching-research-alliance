...
import { ManagedReferralCompletionPanel } from "./managed-referral-completion-panel.js";
...
import { CampusRegionAssignmentPanel } from "./campus-region-assignment-panel.js";
...
type Page = "fees" | "overview" | "referrals" | "referral-administration" | "withdrawals" | "finance" | "purchase" | "purchase-history" | "funds" | "accounts" | "people" | "reimbursements" | "reimbursement-history" | "refunds" | "refund-history" | "venue-board" | "venue-management" | "salary" | "salary-confirmation" | "bonus-projects" | "benefits" | "organization-revenue" | "group-leader-change" | "teaching-mentor-change" | "admin-planning-mentor-change" | "person-campus-assignment" | "campus-region-assignment" | "person-relationship-audit" | "planning-mentor-relationships";
...
const canCreateReferral = (session: SessionSnapshot | null): boolean => {
  const role = session?.currentRoleContext?.subject as string | undefined;
...
export function App(): ReactNode {
  ...
  const financeUnconfirmed = referralUnconfirmed || relationshipUnconfirmed || accountUnconfirmed || peopleUnconfirmed || benefitConfirmationUnconfirmed || benefitPlanUnconfirmed || wageConfirmationUnconfirmed || bonusUnconfirmed || grantUnconfirmed || wagePlanUnconfirmed || withdrawalUnconfirmed || purchaseUnconfirmed || fundUnconfirmed || venueUnconfirmed || reimbursementUnconfirmed || refundUnconfirmed;
  ...
  const load = async (): Promise<void> => {
    ...
    const loadVenueBoard = ["TEACHER", "TEACHING_TEACHER", "ACADEMIC_PLANNER", "PLANNING_MENTOR", "VENUE_OWNER"].includes(role ?? "");
  ...
  const canManageAccountAccess = canManageAccounts(session);
  const canManagePeople = canManagePersonnel(session);
  ...
  const canManagePlanningMentorRelationships = currentRole === "PLANNING_MENTOR" && context?.scope === "SELF"
    && context.regionId === undefined && context.campusId === undefined && context.venueId === undefined;
  const canManageReferrals = canManageRelationships;
  ...
  const page = activePage === "accounts" && canManageAccountAccess ? "accounts"
    ...
    : activePage === "campus-region-assignment" && canManageRelationships ? "campus-region-assignment"
    ...
    : currentRole === "TEACHING_TEACHER" ? (["fees", "overview", "referrals", "withdrawals", "reimbursements", "venue-board"].includes(activePage) ? activePage : "fees")
    : currentRole === "TEACHER" ? (["overview", "withdrawals", "reimbursements", "venue-board"].includes(activePage) ? activePage : "overview")
  ...
  const navigation = <>
    ...
    {canManageRelationships && <button disabled={busy || financeUnconfirmed} aria-current={page === "campus-region-assignment" ? "page" : undefined} onClick={() => goPage("campus-region-assignment")}><DashboardIcon name="accounts" />校区分区调整</button>}
    ...
    {canManagePeople && <button disabled={busy || financeUnconfirmed} aria-current={page === "people" ? "page" : undefined} onClick={() => goPage("people")}><DashboardIcon name="accounts" />人员与职责</button>}
    ...
    <div className="shell">
      ...
      <main>
        ...
        <>
            ...
            {canManageRelationships && <div hidden={page !== "campus-region-assignment"}><CampusRegionAssignmentPanel client={client} session={session} sessionKey={financeKey} busy={busy} onUnconfirmedChange={setRelationshipUnconfirmed} onSaved={() => setOverviewFresh(false)} onInvalidated={() => { clear(); setSession(client.currentSession); setMessage("登录或身份已失效，请重新登录或选择身份。"); }} /></div>}
