...
test('管理员维护奖金名称并持久化，丢失响应后同命令重试',async({page})=>{
 ...
 await login(page,'13800000004');await page.getByRole('button',{name:'项目奖金',exact:true}).click();
 ...
 await expect(page.getByRole('button',{name:'工资管理',exact:true})).toBeDisabled();await expect(panel).toBeVisible();
...
test('总部财务只读奖金目录，个人教师没有入口',async({page})=>{
 test.skip(process.env.ALLIANCE_SYNTHETIC_E2E!=='1','Requires isolated synthetic demo');
 await login(page,'13800000003');await page.getByLabel('当前身份',{exact:true}).selectOption('HEADQUARTERS_FINANCE');await page.getByRole('button',{name:'项目奖金',exact:true}).click();
 ...
 await page.getByLabel('当前身份',{exact:true}).selectOption('TEACHING_TEACHER');await expect(page.getByRole('button',{name:'项目奖金',exact:true})).toHaveCount(0);await expect(panel).toHaveCount(0);
});
