...
export const FULL_BACKUP_LAYOUT_VERSION = "raw-source-layout.v9";
...
const groups = [
  ["01", "组织账户与权限", "organization_unit campus_region_assignment campus_region_assignment_change_preview campus_region_assignment_change campus_region_assignment_person_effect campus_region_assignment_settlement_effect person_campus_assignment person_campus_assignment_change_preview person_campus_assignment_change person_campus_assignment_change_effect person_responsibility_command person_relationship person_relationship_change person_relationship_change_effect person_relationship_change_preview admin_planning_mentor_relationship_change admin_planning_mentor_relationship_change_effect admin_planning_mentor_relationship_change_preview planning_mentor_relationship_change planning_mentor_relationship_change_effect planning_mentor_relationship_change_preview teaching_mentor_relationship_change teaching_mentor_relationship_change_effect teaching_mentor_relationship_change_preview role_assignment settlement_account company_finance_fund company_finance_fund_assignment company_finance_fund_command_idempotency user_account user_session auth_login_throttle auth_password_reset_command venue venue_command_idempotency venue_permission_grant"],
  ["05", "后台任务", "background_task background_task_attempt"],
  ["02", "教师信息", "person person_profile_change teacher_profile teacher_profile_identity_change"],
...
export function createFullBackupLayout(): readonly FullBackupLayoutItem[] {
  ...
  return EXPORT_SCHEMA_REGISTRY.map((table, index) => {
    ...
    return {
      ...
      // Excel sheet names must leave room for the spool part suffix.  The
      // inventory now exceeds 99 tables, so reserve one extra index digit.
      sheetId: excluded ? null : `t${String(index + 1).padStart(2, "0")}_${table.name.slice(0, 20)}`,
