...
const meta = { spoolId: "spool", snapshotId: "snapshot", asOf: "2026-09-23T00:00:00.000Z" };
...
test("table-7 workbook pages bounded streams, preserves exact beans and keeps unsupported business periods explicit", async () => {
  ...
  const index = { metadata: () => meta, lookup: async () => undefined };
  const ledger = {
  ...
  try {
    const result = await new FullBackupLedgerWorkbook({ index, ledger, periods, outputRoot: root, maxDataRows: 1 }).export();
