...
test('every registered table has exactly one explicit group and secrets-only tables have no sheet',()=>{
 const items=createFullBackupLayout();assert.equal(items.length,101);assert.equal(new Set(items.map(item=>item.tableName)).size,101);
 ...
 for(const tableName of ['auth_login_throttle','auth_password_reset_command']){
  const item=items.find(candidate=>candidate.tableName===tableName);
  assert.equal(item.sheetId,null);assert.equal(item.policy,'AUTH_SECRET_TABLE_EXCLUDED');assert.ok(item.excludedColumns.length>0);
 }
 ...
 assert.equal(items.some(item=>item.workbookId==='05'),true);assert.ok(FULL_BACKUP_KNOWN_COVERAGE_GAPS.includes('MONTHLY_INCOME_PUBLISHED_VERSIONS_NOT_IMPLEMENTED'));
