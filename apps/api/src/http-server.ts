    ...
    try {
      ...
      if (request.method === "GET" && pathname === "/health") {
        writeJson(response, 200, {
          service: "teaching-research-alliance-api",
          status: "ok",
        });
        return;
      }
      ...
      const contentPath = pathname.match(
        /^\/v1\/finance\/attachment-uploads\/([^/]+)\/content$/,
      );
      if (method === "POST" && contentPath !== null) {
        // This binary route bypasses the JSON parser and its separate 1 MiB limit.
        response.setHeader("connection", "close");
        const deadline = setTimeout(
          () => request.destroy(new Error("ATTACHMENT_UPLOAD_TIMEOUT")),
          30_000,
        );
        try {
          ...
          if (!session.currentRoleContext)
            throw new Error("ROLE_CONTEXT_REQUIRED");
          if (!services.financeAttachmentUploads)
          ...
          const data = await services.financeAttachmentUploads.upload(
            session.currentRoleContext,
            contentPath[1]!,
            request,
            at,
          );
      ...
      const downloadPath = pathname.match(
        /^\/v1\/finance\/attachments\/([^/]+)\/content$/,
      );
      if (method === "GET" && downloadPath !== null) {
        ...
        try {
          ...
          if (!session.currentRoleContext)
            throw new Error("ROLE_CONTEXT_REQUIRED");
          if (!services.financeAttachmentReads)
            throw new Error("ATTACHMENT_STORAGE_UNAVAILABLE");
          const data = await services.financeAttachmentReads.readOwn(
            session.currentRoleContext,
            downloadPath[1]!,
            at,
          );
          const filename = encodeURIComponent(data.originalFilename).replace(
            /['()*]/g,
