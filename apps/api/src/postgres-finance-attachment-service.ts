...
const versionSelect = `
  SELECT attachment.id::text AS attachment_id,version.id::text AS version_id,version.version_no::text AS version_no,version.status,
         ...
         to_char(COALESCE(withdrawal_submission.submitted_at,self_purchase.completed_at,reimbursement_submission.submitted_at,refund_submission.submitted_at,document.created_at) AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS business_at
    ...
    LEFT JOIN finance_refund_submission refund_submission ON refund_submission.finance_document_id=document.id
        ...
        `SELECT document.id::text AS document_id,document.applicant_person_id::text AS applicant_person_id,
           ...
           LEFT JOIN (
             SELECT finance_document_id,finance_attachment_version_id,stage,document_version,bound_at
               FROM finance_withdrawal_attachment_binding
             UNION ALL
