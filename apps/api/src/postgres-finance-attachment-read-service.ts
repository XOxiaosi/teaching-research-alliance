...
const isAllowedSubject = (subject: string): boolean =>
  allowedSubjects.includes(subject as (typeof allowedSubjects)[number]);

const isMediaType = (value: string | null): value is AttachmentMediaType =>
  value !== null && mediaTypes.includes(value as AttachmentMediaType);
...
const isReadyAt = (value: string | null): boolean =>
...
/** Reads only the caller's already READY original; it neither reparses nor changes attachment or ledger state. */
export class PostgresFinanceAttachmentReadService {
  ...
  public async readOwn(context: RoleContext, versionId: string, at: Date): Promise<FinanceAttachmentDownload> {
    ...
    try {
      ...
      if (!isAllowedSubject(context.subject)) {
      ...
      const result = await client.query<AttachmentRow>(
        `SELECT version.id::text AS version_id,
          ...
          WHERE version.id=$1::uuid AND document.applicant_person_id=$2::uuid
      ...
      if (row.status !== "READY" || !isMediaType(row.detected_media_type) || sizeBytes === undefined
        || row.sha256 === null || !SHA256.test(row.sha256) || !isReadyAt(row.ready_at)) {
        return await failAfterAudit("ATTACHMENT_INTEGRITY_FAILED", "READY_METADATA_INVALID", "ATTACHMENT_INTEGRITY_FAILED");
      }

      let bytes: Buffer;
      try {
        bytes = await this.store.readVerified({
