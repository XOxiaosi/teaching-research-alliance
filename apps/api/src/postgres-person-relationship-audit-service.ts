...
const REPAIRABILITIES = ["GROUP_LEADER_REGULAR_WEEK_PREVIEW", "TEACHING_MENTOR_REGULAR_WEEK_PREVIEW", "READ_ONLY", "REQUIRES_RELATIONSHIP_CORRECTION", "REQUIRES_P27", "REQUIRES_SPECIAL_PERIOD_SCOPE"] as const;
...
type Row = {
  ...
  created_by_id: string | null; created_by_nickname: string | null; created_at: string | null; superseded_by_change_id: string | null; superseded_by_teaching_mentor_change_id: string | null; superseded_by_planning_mentor_change_id: string | null;
  role_ids: string[]; status: PersonRelationshipAuditStatus; anomaly_codes: PersonRelationshipAnomalyCode[];
  source_change_kind: "GROUP_LEADER_CHANGE" | "TEACHING_MENTOR_CHANGE" | "PLANNING_MENTOR_CHANGE" | null; source_change_id: string | null;
  ...
  public async list(context: RoleContext, filter: PersonRelationshipAuditFilter, at: Date): Promise<PersonRelationshipAuditPage> {
    ...
    try {
      ...
      const anomalies = `array_remove(ARRAY[
        ...
        CASE WHEN r.superseded_at IS NOT NULL AND r.superseded_at <= $1::timestamptz AND ((r.relationship_type='PLANNING_MENTOR' AND NOT EXISTS (SELECT 1 FROM planning_mentor_relationship_change c WHERE c.id=r.superseded_by_planning_mentor_change_id AND c.source_relationship_id=r.id)) OR (r.relationship_type='TEACHING_MENTOR' AND NOT EXISTS (SELECT 1 FROM teaching_mentor_relationship_change c WHERE c.id=r.superseded_by_teaching_mentor_change_id AND c.source_relationship_id=r.id)) OR (r.relationship_type='GROUP_LEADER' AND NOT EXISTS (SELECT 1 FROM person_relationship_change c WHERE c.id=r.superseded_by_change_id AND c.source_relationship_id=r.id)) OR (r.relationship_type='CAMPUS_PRINCIPAL')) THEN 'HISTORICAL_REFERENCE_MISSING' END
      ]::text[], NULL)`;
      const base = `SELECT r.id::text audit_item_id,r.id::text relationship_id,encode(digest(concat_ws('|',r.id::text,r.relationship_type,r.teacher_id::text,r.related_person_id::text,r.valid_from::text,COALESCE(r.valid_to::text,''),COALESCE(r.effective_scope,''),CASE WHEN r.superseded_at <= $1::timestamptz THEN r.superseded_at::text ELSE '' END),'sha256'),'hex') relationship_fingerprint,r.relationship_type,p.id::text member_id,p.nickname member_nickname,p.status member_status,related.id::text related_id,related.nickname related_nickname,related.status related_status,r.valid_from::text valid_from,r.valid_to::text valid_to,r.effective_scope,creator.id::text created_by_id,creator.nickname created_by_nickname,r.created_at::text created_at,CASE WHEN r.superseded_at <= $1::timestamptz THEN r.superseded_by_change_id::text END superseded_by_change_id,CASE WHEN r.superseded_at <= $1::timestamptz THEN r.superseded_by_teaching_mentor_change_id::text END superseded_by_teaching_mentor_change_id,CASE WHEN r.superseded_at <= $1::timestamptz THEN r.superseded_by_planning_mentor_change_id::text END superseded_by_planning_mentor_change_id,${roleIds} role_ids,${status} status,${anomalies} anomaly_codes,CASE WHEN r.superseded_at <= $1::timestamptz AND r.superseded_by_change_id IS NOT NULL THEN 'GROUP_LEADER_CHANGE' WHEN r.superseded_at <= $1::timestamptz AND r.superseded_by_teaching_mentor_change_id IS NOT NULL THEN 'TEACHING_MENTOR_CHANGE' WHEN r.superseded_at <= $1::timestamptz AND r.superseded_by_planning_mentor_change_id IS NOT NULL THEN 'PLANNING_MENTOR_CHANGE' END source_change_kind,CASE WHEN r.superseded_at <= $1::timestamptz THEN COALESCE(r.superseded_by_change_id,r.superseded_by_teaching_mentor_change_id,r.superseded_by_planning_mentor_change_id)::text END source_change_id,(SELECT count(DISTINCT snap.weekly_fee_entry_id) FROM weekly_fee_allocation_snapshot snap WHERE snap.created_at <= $1::timestamptz AND (CASE r.relationship_type WHEN 'GROUP_LEADER' THEN snap.context_json#>>'{relationships,groupLeader,id}' WHEN 'TEACHING_MENTOR' THEN snap.context_json#>>'{relationships,teachingMentor,id}' WHEN 'PLANNING_MENTOR' THEN snap.context_json#>>'{relationships,planningMentor,id}' ELSE NULL END)=r.id::text)::text weekly_fee_count,(SELECT count(*) FROM weekly_fee_allocation_snapshot snap WHERE snap.created_at <= $1::timestamptz AND (CASE r.relationship_type WHEN 'GROUP_LEADER' THEN snap.context_json#>>'{relationships,groupLeader,id}' WHEN 'TEACHING_MENTOR' THEN snap.context_json#>>'{relationships,teachingMentor,id}' WHEN 'PLANNING_MENTOR' THEN snap.context_json#>>'{relationships,planningMentor,id}' ELSE NULL END)=r.id::text)::text snapshot_count,(SELECT count(*) FROM referral_creation_snapshot ref WHERE r.relationship_type='PLANNING_MENTOR' AND ref.planning_mentor_relationship_id=r.id AND ref.created_at <= $1::timestamptz)::text referral_count FROM person_relationship r JOIN person p ON p.id=r.teacher_id JOIN person related ON related.id=r.related_person_id JOIN person creator ON creator.id=r.created_by LEFT JOIN teacher_profile profile ON profile.person_id=r.teacher_id WHERE ${where.join(" AND ")}`;
      ...
      const page = rows.slice(0, limit); const items = page.map(row => ({
        ...
        repairability: row.repairability, repairBlockedReason: ["GROUP_LEADER_REGULAR_WEEK_PREVIEW", "TEACHING_MENTOR_REGULAR_WEEK_PREVIEW"].includes(row.repairability) ? null : "该关系只能进入只读审计或后续专门纠正流程",
