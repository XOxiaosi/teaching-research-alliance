...
test("真实 PostgreSQL 的 SKIP LOCKED 将并发领取和被锁记录分开", async () => {
  ...
  try {
    ...
    try {
      ...
      const claimed = await queue.claim(at, 60_000, 2);
      assert.deepEqual(claimed.map((item) => item.task.id), [second.id]);
      await locked.query("ROLLBACK");
    } finally { locked.release(); }
    const [a, b] = await Promise.all([queue.claim(later(1000), 60_000), queue.claim(later(1000), 60_000)]);
...
test("过期租约可恢复，旧执行者不能覆盖新领取结果", async () => {
  ...
  try {
    ...
    const oldLease = (await queue.claim(at, 1_000))[0];
    assert.ok(oldLease);
    await assert.rejects(queue.succeed(task.id, oldLease.leaseToken, later(1_001)), /TASK_LEASE_LOST/);
    const currentLease = (await queue.claim(later(1_001), 1_000))[0];
...
test("失败原因、延后重试和停机后耗尽租约均持久可核对", async () => {
  ...
  try {
    ...
    const first = (await queue.claim(at, 1_000))[0]; assert.ok(first);
    ...
    assert.deepEqual(await queue.claim(later(4_999), 1_000), []);
    const second = (await queue.claim(later(5_000), 1_000))[0]; assert.ok(second);
    ...
    const abandoned = (await queue.claim(at, 1_000))[0]; assert.ok(abandoned);
    assert.deepEqual(await queue.claim(later(1_001), 1_000), []);
