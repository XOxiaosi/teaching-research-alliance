...
type Props = Readonly<{ client: TeacherApiClient; venues: readonly MiniVenueBoardOption[]; weeks?: readonly MiniVenueBoardWeek[]; initialVenueId?: string | null | undefined; busy?: boolean; sessionKey?: string; onInvalidated?: () => void }>;
...
export function VenueBoardPanel({ client, venues, weeks = [], initialVenueId, busy = false, sessionKey = "", onInvalidated }: Props): ReactNode {
  ...
  useEffect(() => { const sessionChanged = previousSessionKey.current !== sessionKey; previousSessionKey.current = sessionKey; requestGeneration.current += 1; setBoard(null); setNotice(""); setLoading(false); if (sessionChanged) { setWeekId(""); setStartsOn(""); setEndsOn(""); } if (initialVenueId !== undefined && initialVenueId !== null) setVenueId(initialVenueId); else if (venues.length > 0 && !venues.some((venue) => venue.id === venueId)) setVenueId(venues[0]!.id); else if (venues.length === 0) setVenueId(""); return () => { requestGeneration.current += 1; }; }, [sessionKey, initialVenueId, venues]);
  const loadBoard = async (): Promise<void> => {
    ...
    try { const next = await client.getVenueBoard<MiniVenueBoardData>(venueId, weekId !== "" ? { teachingWeekId: weekId } : { startsOn, endsOn }); if (generation !== requestGeneration.current) return; setBoard(next); } catch (error) { if (generation !== requestGeneration.current) return; if (error instanceof ApiClientError && (error.status === 401 || error.status === 403)) { setBoard(null); onInvalidated?.(); return; } setBoard(null); setNotice(miniBoardErrorMessage(error)); } finally { if (generation === requestGeneration.current) setLoading(false); }
  };
  ...
  return <View className="panel venue-board-panel"><Text className="panel-title">共享场地看板</Text><Text className="panel-description">查看场地内授课老师、学生和每周场地费。查看权限不会显示账户余额。</Text>
    <Text className="field-label">场地</Text><Picker mode="selector" range={venues.map((venue) => `${venue.name}${venue.isOwn ? "（我的场地）" : ""}`)} value={Math.max(venues.findIndex((venue) => venue.id === venueId), 0)} disabled={busy || loading || initialVenueId !== undefined && initialVenueId !== null} onChange={(event) => { requestGeneration.current += 1; setVenueId(venues[Number(event.detail.value)]?.id ?? ""); setBoard(null); setNotice(""); setLoading(false); }}><View className="picker-value"><Text>{board?.venue.name ?? venues.find((venue) => venue.id === venueId)?.name ?? (initialVenueId ? "当前场地" : "请选择场地")}</Text><Text>⌄</Text></View></Picker>
    {weeks.length > 0 && <><Text className="field-label">教学周</Text><Picker mode="selector" range={["按日期筛选", ...weeks.map((week) => week.periodLabel ?? `${week.startsOn} 至 ${week.endsOn}`)]} value={weekId === "" ? 0 : Math.max(weeks.findIndex((week) => week.weekId === weekId) + 1, 0)} disabled={busy || loading} onChange={(event) => { requestGeneration.current += 1; const week = weeks[Number(event.detail.value) - 1]; setWeekId(week?.weekId ?? ""); if (week) { setStartsOn(""); setEndsOn(""); } setBoard(null); setNotice(""); setLoading(false); }}><View className="picker-value"><Text>{weeks.find((week) => week.weekId === weekId)?.periodLabel ?? (weekId ? "已选教学周" : "按日期筛选")}</Text><Text>⌄</Text></View></Picker></>}
    {weekId === "" && <><Text className="field-label">开始日期</Text><Input className="text-input" type="text" value={startsOn} placeholder="YYYY-MM-DD" disabled={busy || loading} onInput={(event) => { requestGeneration.current += 1; setStartsOn(event.detail.value); setBoard(null); setNotice(""); }} /><Text className="field-label">结束日期</Text><Input className="text-input" type="text" value={endsOn} placeholder="YYYY-MM-DD" disabled={busy || loading} onInput={(event) => { requestGeneration.current += 1; setEndsOn(event.detail.value); setBoard(null); setNotice(""); }} /></>}
