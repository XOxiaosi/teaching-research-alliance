...
import { ProjectBonusHistoryPanel } from "./project-bonus-history-panel";
...
import { AdminPlanningMentorRelationshipPanel } from "./admin-planning-mentor-relationship-panel";
import { PersonCampusAssignmentPanel } from "./person-campus-assignment-panel";
import { CampusRegionAssignmentPanel } from "./campus-region-assignment-panel";
    ...
    <ScrollView scrollY className="page-shell">
      ...
      {session === null ? (
        ...
        <>
          ...
          {canManageRelationships && <AdminPlanningMentorRelationshipPanel client={client} session={session} sessionKey={`${session.sessionId}:${JSON.stringify(currentContext)}`} busy={busy} onUnconfirmedChange={setRelationshipUnconfirmed} onSaved={() => setOverview(null)} onInvalidated={() => { clearTeachingState(); setSession(client.currentSession); setNotice("登录或身份已失效，请重新登录或选择身份。"); }} />}
          {canManageRelationships && <PersonCampusAssignmentPanel client={client} session={session} sessionKey={`${session.sessionId}:${JSON.stringify(currentContext)}`} busy={busy} onUnconfirmedChange={setRelationshipUnconfirmed} onSaved={() => setOverview(null)} onInvalidated={() => { clearTeachingState(); setSession(client.currentSession); setNotice("登录或身份已失效，请重新登录或选择身份。"); }} />}
          {canManageRelationships && <CampusRegionAssignmentPanel client={client} session={session} sessionKey={`${session.sessionId}:${JSON.stringify(currentContext)}`} busy={busy} onUnconfirmedChange={setRelationshipUnconfirmed} onSaved={() => setOverview(null)} onInvalidated={() => { clearTeachingState(); setSession(client.currentSession); setNotice("登录或身份已失效，请重新登录或选择身份。"); }} />}
          ...
          {canReadSalary && <ProjectBonusGrantPanel client={client} session={session} sessionKey={`${session.sessionId}:${JSON.stringify(currentContext)}`} busy={busy || bonusUnconfirmed} onUnconfirmedChange={setBonusGrantUnconfirmed} onSaved={() => setBonusHistoryRevision(value => value + 1)} onInvalidated={() => { clearTeachingState(); setSession(null); setNotice("登录或身份已失效，请重新登录。"); }} />}
          ...
          {canReadSalary && <ProjectBonusHistoryPanel client={client} session={session} sessionKey={`${session.sessionId}:${JSON.stringify(currentContext)}`} revision={bonusHistoryRevision} busy={busy || bonusGrantUnconfirmed} onInvalidated={() => { clearTeachingState(); setSession(client.currentSession); setNotice("登录或身份已失效，请重新登录或选择身份。"); }} />}
