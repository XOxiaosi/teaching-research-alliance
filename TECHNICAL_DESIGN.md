...
`rate_policy_version` 保存政策版本、有效期、基础费率、上限、状态和创建依据；`rate_tier` 保存上下界、包含方向、抬升比例和是否执行。发布前执行区间检查：重叠、缺口、反向边界或未执行范围均生成阻塞错误。上限默认16%，管理员可修改；计算采用对应生效政策版本中的上限，变更生效范围按D14决定。D06剩余年度规则、D14或D18尚未确定且影响本次计算时，系统可以保存原始配置和进行标注，但不得对受影响月份执行最终结算。
...
表7最终结算余额（本次按用户所称“欢乐豆余额”记录）使用可带符号的精确十进制值，允许小于零；数据库、API 和客户端不得施加余额非负约束或使用 `max(balance, 0)`。负余额在看板和导出中保留，不能当成单笔分配超额错误；提现资格与可提现额度另按 D10/D16 决定。
...
- 费率区间边界、默认16%及修改后上限封顶、比例上限、缺失收款对象、正负金额和跨月案例有确定性测试。
...
技术设计不替用户决定以下业务口径：费率区间缺口/重叠、净课时费定义、上限修改的生效范围、两类角色分配、提现有效字段、单据何时计入余额、缺失收款对象、舍入尾差、周流水生成与退费、关账重算、多角色范围、真实资金流程、手机改名交互、自动备份/附件保留和普通用户网页写入范围。它们必须在 PRODUCT 中形成决定后，才转为代码和测试常量。
