-- DEV-009A: versioned ordinary-week group-leader changes rebind only the
-- effective groupLeader allocation account while preserving every prior fact.

ALTER TABLE person_relationship
  ADD COLUMN superseded_at timestamptz,
  ADD COLUMN superseded_by_change_id uuid,
  ADD CONSTRAINT person_relationship_supersession_pair CHECK (
    (superseded_at IS NULL AND superseded_by_change_id IS NULL)
    OR (superseded_at IS NOT NULL AND superseded_by_change_id IS NOT NULL)
  );

ALTER TABLE person_relationship
  DROP CONSTRAINT person_relationship_teacher_id_relationship_type_tstzrange_excl;
ALTER TABLE person_relationship
  ADD CONSTRAINT person_relationship_active_period_no_overlap
  EXCLUDE USING gist (
    teacher_id WITH =,
...
CREATE TABLE person_relationship_change_preview (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  relationship_type text NOT NULL CHECK (relationship_type = 'GROUP_LEADER'),
  teacher_person_id uuid NOT NULL REFERENCES person(id),
  source_relationship_id uuid NOT NULL REFERENCES person_relationship(id),
  source_related_person_id uuid NOT NULL REFERENCES person(id),
  ...
  CHECK (source_related_person_id <> new_related_person_id),
  CHECK (next_boundary_at IS NULL OR next_boundary_at > effective_at)
);

CREATE TABLE person_relationship_change (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  preview_id uuid NOT NULL UNIQUE REFERENCES person_relationship_change_preview(id),
  relationship_type text NOT NULL CHECK (relationship_type = 'GROUP_LEADER'),
  teacher_person_id uuid NOT NULL REFERENCES person(id),
  ...
  CHECK (source_relationship_id <> result_relationship_id),
  CHECK (source_related_person_id <> new_related_person_id),
  CHECK (next_boundary_at IS NULL OR next_boundary_at > effective_at),
...
ALTER TABLE person_relationship
  ADD CONSTRAINT person_relationship_superseded_change_fk
  FOREIGN KEY (superseded_by_change_id) REFERENCES person_relationship_change(id)
  DEFERRABLE INITIALLY DEFERRED;

CREATE TABLE person_relationship_change_effect (
  change_id uuid NOT NULL REFERENCES person_relationship_change(id),
  ...
  group_leader_amount_cents bigint NOT NULL CHECK (group_leader_amount_cents > 0),
...
CREATE FUNCTION refuse_person_relationship_change_mutation()
...
CREATE FUNCTION group_leader_timestamp_text(value timestamptz)
...
CREATE FUNCTION group_leader_relationship_fact(row_value person_relationship)
RETURNS jsonb LANGUAGE sql STABLE AS $$
  SELECT jsonb_build_object(
    ...
    'validFrom', group_leader_timestamp_text(row_value.valid_from),
    'validTo', group_leader_timestamp_text(row_value.valid_to),
    'effectiveScope', row_value.effective_scope,
    'createdByPersonId', row_value.created_by::text,
    'createdAt', group_leader_timestamp_text(row_value.created_at),
    'supersededAt', group_leader_timestamp_text(row_value.superseded_at),
    'supersededByChangeId', row_value.superseded_by_change_id::text
  );
$$;

CREATE FUNCTION assert_published_person_relationship_update()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM person_relationship_change relationship_change
     WHERE relationship_change.source_relationship_id=OLD.id
        OR relationship_change.result_relationship_id=OLD.id
  ) AND NOT EXISTS (
    SELECT 1 FROM person_relationship_change transition
     WHERE transition.source_relationship_id=OLD.id
       AND transition.before_json->'sourceRelationship'=group_leader_relationship_fact(OLD)
       AND transition.after_json->'sourceRelationship'=group_leader_relationship_fact(NEW)
  ) THEN
...
CREATE FUNCTION guard_published_person_relationship_delete()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM person_relationship_change relationship_change
...
CREATE FUNCTION guard_person_relationship_change_effect()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  change_row person_relationship_change%ROWTYPE;
  ...
  fee_valid boolean;
BEGIN
  SELECT * INTO change_row FROM person_relationship_change WHERE id=NEW.change_id;
  ...
  IF change_row.id IS NULL OR previous_row.id IS NULL OR result_row.id IS NULL
    ...
    OR line_amount IS DISTINCT FROM NEW.group_leader_amount_cents
    OR previous_row.snapshot_json->'lines' IS DISTINCT FROM result_row.snapshot_json->'lines'
    OR (previous_row.snapshot_json #- '{accountByKey,groupLeader}'::text[])
       IS DISTINCT FROM (result_row.snapshot_json #- '{accountByKey,groupLeader}'::text[])
    OR previous_row.snapshot_json#>>'{accountByKey,groupLeader}' IS DISTINCT FROM source_account.account_code
    OR result_row.snapshot_json#>>'{accountByKey,groupLeader}' IS DISTINCT FROM destination_account.account_code
    OR previous_row.context_json#>>'{relationships,groupLeader,id}' IS DISTINCT FROM change_row.source_relationship_id::text
    OR previous_row.context_json#>>'{relationships,groupLeader,personId}' IS DISTINCT FROM change_row.source_related_person_id::text
    OR previous_row.context_json#>>'{accounts,groupLeader,ownerType}' IS DISTINCT FROM 'PERSON'
    OR previous_row.context_json#>>'{accounts,groupLeader,ownerId}' IS DISTINCT FROM change_row.source_related_person_id::text
    OR previous_row.context_json#>>'{accounts,groupLeader,accountId}' IS DISTINCT FROM source_account.id::text
    OR previous_row.context_json#>>'{accounts,groupLeader,accountCode}' IS DISTINCT FROM source_account.account_code
    OR (previous_row.context_json #- '{relationships,groupLeader}'::text[] #- '{accounts,groupLeader}'::text[])
       IS DISTINCT FROM (result_row.context_json #- '{relationships,groupLeader}'::text[] #- '{accounts,groupLeader}'::text[])
    OR result_row.context_json#>>'{relationships,groupLeader,id}' IS DISTINCT FROM change_row.result_relationship_id::text
    OR result_row.context_json#>>'{relationships,groupLeader,personId}' IS DISTINCT FROM change_row.new_related_person_id::text
    OR result_row.context_json#>>'{accounts,groupLeader,ownerType}' IS DISTINCT FROM 'PERSON'
    OR result_row.context_json#>>'{accounts,groupLeader,ownerId}' IS DISTINCT FROM change_row.new_related_person_id::text
    OR result_row.context_json#>>'{accounts,groupLeader,accountId}' IS DISTINCT FROM destination_account.id::text
    OR result_row.context_json#>>'{accounts,groupLeader,accountCode}' IS DISTINCT FROM destination_account.account_code
...
CREATE FUNCTION assert_person_relationship_change_complete()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  ...
  preview_row person_relationship_change_preview%ROWTYPE;
  ...
  after_result jsonb;
BEGIN
  SELECT * INTO preview_row FROM person_relationship_change_preview WHERE id=NEW.preview_id;
  ...
  IF NOT EXISTS (
    SELECT 1 FROM person_relationship_change_preview preview
     WHERE preview.id=NEW.preview_id AND preview.relationship_type=NEW.relationship_type
       AND preview.teacher_person_id=NEW.teacher_person_id
       AND preview.source_relationship_id=NEW.source_relationship_id
       AND preview.source_related_person_id=NEW.source_related_person_id
    ...
    SELECT 1 FROM person_relationship relationship
     WHERE relationship.id=NEW.result_relationship_id
       ...
       AND relationship.superseded_by_change_id IS NULL
  ) OR NOT EXISTS (
  ...
  IF preview_row.id IS NULL OR source_row.id IS NULL OR result_row.id IS NULL OR week_row.id IS NULL
    ...
    OR source_row.teacher_id IS DISTINCT FROM NEW.teacher_person_id
    ...
    OR jsonb_typeof(before_source) IS DISTINCT FROM 'object'
    OR jsonb_typeof(after_source) IS DISTINCT FROM 'object'
    OR jsonb_typeof(after_result) IS DISTINCT FROM 'object'
    OR (SELECT count(*) FROM jsonb_object_keys(NEW.before_json))<>1
    OR (SELECT count(*) FROM jsonb_object_keys(NEW.after_json))<>2
    OR (SELECT count(*) FROM jsonb_object_keys(before_source))<>11
    OR (SELECT count(*) FROM jsonb_object_keys(after_source))<>11
    OR (SELECT count(*) FROM jsonb_object_keys(after_result))<>11
    ...
    OR before_source->>'validFrom' IS DISTINCT FROM group_leader_timestamp_text(source_row.valid_from)
    OR before_source->>'validTo' IS DISTINCT FROM group_leader_timestamp_text(NEW.next_boundary_at)
    ...
    OR before_source->>'createdAt' IS DISTINCT FROM group_leader_timestamp_text(source_row.created_at)
    OR before_source->>'supersededAt' IS NOT NULL
    OR before_source->>'supersededByChangeId' IS NOT NULL
    OR after_source->>'id' IS DISTINCT FROM source_row.id::text
    OR after_source->>'teacherPersonId' IS DISTINCT FROM source_row.teacher_id::text
    OR after_source->>'relationshipType' IS DISTINCT FROM source_row.relationship_type
    OR after_source->>'relatedPersonId' IS DISTINCT FROM source_row.related_person_id::text
    OR after_source->>'validFrom' IS DISTINCT FROM group_leader_timestamp_text(source_row.valid_from)
    OR after_source->>'validTo' IS DISTINCT FROM group_leader_timestamp_text(source_row.valid_to)
    OR after_source->>'effectiveScope' IS DISTINCT FROM source_row.effective_scope
    OR after_source->>'createdByPersonId' IS DISTINCT FROM source_row.created_by::text
    OR after_source->>'createdAt' IS DISTINCT FROM group_leader_timestamp_text(source_row.created_at)
    OR after_source->>'supersededAt' IS DISTINCT FROM group_leader_timestamp_text(source_row.superseded_at)
    OR after_source->>'supersededByChangeId' IS DISTINCT FROM source_row.superseded_by_change_id::text
    OR after_result->>'id' IS DISTINCT FROM result_row.id::text
    OR after_result->>'teacherPersonId' IS DISTINCT FROM result_row.teacher_id::text
    OR after_result->>'relationshipType' IS DISTINCT FROM result_row.relationship_type
    OR after_result->>'relatedPersonId' IS DISTINCT FROM result_row.related_person_id::text
    OR after_result->>'validFrom' IS DISTINCT FROM group_leader_timestamp_text(result_row.valid_from)
    OR after_result->>'validTo' IS DISTINCT FROM group_leader_timestamp_text(result_row.valid_to)
    OR after_result->>'effectiveScope' IS DISTINCT FROM result_row.effective_scope
    OR after_result->>'createdByPersonId' IS DISTINCT FROM result_row.created_by::text
    OR after_result->>'createdAt' IS DISTINCT FROM group_leader_timestamp_text(result_row.created_at)
    OR after_result->>'supersededAt' IS NOT NULL
    OR after_result->>'supersededByChangeId' IS NOT NULL
    OR NOT (
      (source_row.valid_from<NEW.effective_at AND source_row.valid_to=NEW.effective_at
       AND source_row.superseded_at IS NULL AND source_row.superseded_by_change_id IS NULL)
      OR
      (source_row.valid_from=NEW.effective_at
       AND source_row.valid_to IS NOT DISTINCT FROM NEW.next_boundary_at
       AND source_row.superseded_at=NEW.published_at AND source_row.superseded_by_change_id=NEW.id)
    ) THEN
    RAISE EXCEPTION 'PERSON_RELATIONSHIP_CHANGE_INCOMPLETE';
  END IF;

  SELECT count(*), COALESCE(sum(group_leader_amount_cents),0)
    INTO effect_count,effect_total
    FROM person_relationship_change_effect WHERE change_id=NEW.id;
  ...
  IF run_snapshot_count<>effect_count
    OR EXISTS (
      SELECT 1 FROM weekly_fee_allocation_snapshot snapshot
       WHERE snapshot.run_id=NEW.settlement_calculation_run_id
         AND NOT EXISTS (
           SELECT 1 FROM person_relationship_change_effect effect
    ...
    OR NOT EXISTS (
      ...
      JOIN person_relationship_change_effect effect
  ...
  SELECT count(*) INTO expected_entries FROM (
    SELECT source_account_id AS account_id FROM person_relationship_change_effect WHERE change_id=NEW.id
    UNION
    SELECT destination_account_id AS account_id FROM person_relationship_change_effect WHERE change_id=NEW.id
  ) accounts;
    ...
    OR EXISTS (
      SELECT 1 FROM (
        SELECT account_id, sum(amount_cents) amount_cents
          FROM ledger_entry WHERE event_id=NEW.ledger_event_id AND category_key='groupLeader'
         GROUP BY account_id
      ) actual FULL JOIN (
        SELECT account_id, sum(amount_cents) amount_cents FROM (
          SELECT source_account_id account_id, -group_leader_amount_cents amount_cents
            FROM person_relationship_change_effect WHERE change_id=NEW.id
          UNION ALL
          SELECT destination_account_id account_id, group_leader_amount_cents amount_cents
            FROM person_relationship_change_effect WHERE change_id=NEW.id
  ...
  IF EXISTS (
    SELECT 1
      FROM (
        SELECT source_account_id account_id FROM person_relationship_change_effect WHERE change_id=NEW.id
        UNION
        SELECT destination_account_id account_id FROM person_relationship_change_effect WHERE change_id=NEW.id
      ) affected
...
CREATE FUNCTION assert_person_relationship_change_effect_parent()
RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE
  change_row person_relationship_change%ROWTYPE;
  effect_count integer;
...
BEGIN
  ...
  IF change_row.id IS NULL THEN
    RAISE EXCEPTION 'PERSON_RELATIONSHIP_CHANGE_INCOMPLETE';
  END IF;
  SELECT count(*) INTO effect_count FROM person_relationship_change_effect WHERE change_id=NEW.change_id;
...
CREATE FUNCTION guard_person_relationship_change_snapshot_append()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM person_relationship_change relationship_change
...
CREATE FUNCTION guard_person_relationship_change_ledger_append()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM person_relationship_change relationship_change
...
CREATE TRIGGER person_relationship_change_preview_immutable
BEFORE UPDATE OR DELETE ON person_relationship_change_preview
FOR EACH ROW EXECUTE FUNCTION refuse_person_relationship_change_mutation();
CREATE TRIGGER person_relationship_change_immutable
BEFORE UPDATE OR DELETE ON person_relationship_change
FOR EACH ROW EXECUTE FUNCTION refuse_person_relationship_change_mutation();
CREATE TRIGGER person_relationship_change_effect_guard
BEFORE INSERT ON person_relationship_change_effect
FOR EACH ROW EXECUTE FUNCTION guard_person_relationship_change_effect();
CREATE TRIGGER person_relationship_change_effect_immutable
BEFORE UPDATE OR DELETE ON person_relationship_change_effect
FOR EACH ROW EXECUTE FUNCTION refuse_person_relationship_change_mutation();
CREATE CONSTRAINT TRIGGER person_relationship_published_update_complete
AFTER UPDATE ON person_relationship DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION assert_published_person_relationship_update();
CREATE TRIGGER person_relationship_published_delete_guard
BEFORE DELETE ON person_relationship
FOR EACH ROW EXECUTE FUNCTION guard_published_person_relationship_delete();
CREATE CONSTRAINT TRIGGER person_relationship_change_effect_parent_complete
AFTER INSERT ON person_relationship_change_effect DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION assert_person_relationship_change_effect_parent();
CREATE CONSTRAINT TRIGGER person_relationship_change_complete
AFTER INSERT ON person_relationship_change DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION assert_person_relationship_change_complete();
CREATE TRIGGER person_relationship_change_snapshot_terminal
BEFORE INSERT ON weekly_fee_allocation_snapshot
FOR EACH ROW EXECUTE FUNCTION guard_person_relationship_change_snapshot_append();
CREATE TRIGGER person_relationship_change_ledger_terminal
BEFORE INSERT ON ledger_entry
FOR EACH ROW EXECUTE FUNCTION guard_person_relationship_change_ledger_append();

CREATE INDEX person_relationship_active_lookup
  ON person_relationship(teacher_id,relationship_type,valid_from,valid_to)
  WHERE superseded_at IS NULL;
CREATE INDEX person_relationship_change_teacher_lookup
  ON person_relationship_change(teacher_person_id,relationship_type,relationship_version DESC);
CREATE INDEX person_relationship_change_publisher_lookup
  ON person_relationship_change(published_by_person_id,published_at DESC,id);
CREATE INDEX person_relationship_change_effect_fee_lookup
  ON person_relationship_change_effect(weekly_fee_entry_id,created_at DESC,change_id);
